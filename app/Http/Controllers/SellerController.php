...
class SellerController extends Controller
{
    ...
    //     // Re-validate user existence in Branch model
    //     $branch = Branch::find($user);
    //     if (!$branch) {
    //         \Log::error('User not found in Branch model for ID: ' . $user);
    ...
    //     // Re-establish session to ensure it persists
    //     Session::put('sid', $user);
    //     Session::save(); // Explicitly save the session to ensure persistence
        ...
        try {
            ...
            if (isset($res['success']) && $res['success'] === true && $res['data']['state'] === 'COMPLETED') {
                ...
                if ($wallet && $wallet->status !== 'success') {
                    ...
                    // Restore user session and login
                    $user = Branch::find($wallet->userid);
                    if ($user) {
                        Auth::login($user);
                        Session::put('sid', $wallet->userid);
                        Session::save();
                    ...
                    Log::warning('Wallet already success or not found for transaction: ' . $merchantTransactionId);
                    return redirect()->to('https://delhiparcel.com/seller-wallet')->with('error', 'Wallet already updated or not found.');
                }
            } else {
                Log::error('Payment failed for transaction: ' . $merchantTransactionId, ['response' => $res]);
